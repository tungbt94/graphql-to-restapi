...
class RouteHandleDefine {
  ...
  defineMutation({ name, args, type }) {
    ...
    this.mutation[name] = function (
      ...
      let queryStr = `mutation ${self._genQuery({
        name,
        args,
        params: params_,
        response,
      ...
      let context =
        typeof formatContext === 'function' ? formatContext(req) : req
      return self
        ._excuteGraphql(queryStr, {}, params_, context)
        .then((result) => {
          return self._formatResult(result)
        })
        .then((data = {}) => {
          let resData =
            typeof formatResponse === 'function'
              ? formatResponse(null, data, routeConfig)
              : {
                  code: 200,
                  message: 'SUCCESS',
                  data,
          ...
          res
            ...
            .send(JSON.stringify(resData, null, 2))
        })
        .catch((err) => {
          let result
  ...
  defineQuery({ name, args, type }) {
    ...
    this.query[name] = function (
      ...
      response = fields || response
      fragment = fields ? '' : fragment
      let queryStr = `query ${self._genQuery({
        name,
        args,
        params: params_,
        response,
  ...
  _genQuery({
    ...
    _.map(args, function (arg) {
      ...
      if (params && name in params) {
        if (
          (type.name === 'Boolean' && type.kind === 'SCALAR') ||
          (type.kind === 'NON_NULL' && type.ofType.name === 'Boolean')
        ) {
          params[name] =
            params[name] === false ||
            params[name] === 'false' ||
        ...
        if (
          (type.name === 'Int' && type.kind === 'SCALAR') ||
          (type.kind === 'NON_NULL' && type.ofType.name === 'Int')
        ) {
